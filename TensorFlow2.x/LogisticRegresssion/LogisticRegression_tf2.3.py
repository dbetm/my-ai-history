"""
Logistic Regression:
One of the most popular classification methods. The purpose of the
logistic regression method is to predict the outcome, wich is categorical.

Logistic regression's underlying method is the same as that for linear
regression. Suppose we take the multi-class linear equation, as shown following:
...
We will apply a logistic function (sigmoid) to the linear equation, as follows:

p(y~=1) = 1 / (1 + e ^-(y = m1x1 + m2x2 + ..., mnxn + b))

Implement the logistic regression method with TensorFlow 2.3, using the iris
data set and the LinearClassifier estimator.
"""
...
logistic_model = LinearClassifier(
